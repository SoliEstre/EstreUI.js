...
| `include` | 테스트 탐색을 `test/` 폴더로 한정. `test/estreui/` 는 프레임워크 테스트, `test/project/` 는 앱 고유 테스트로 분리합니다. |

## 3. 스크립트 로더 (`test/estreui/setup.js`)
...
// test/estreui/setup.js 에서 프로젝트 루트의 scripts/ 로 한 단계 더 올라갑니다
const scriptsDir = resolve(import.meta.dirname, '..', '..', 'scripts');

// index.html의 <script> 로드 순서와 일치해야 함
// (EstreUI.js 는 8개 모듈 파일로 분리되어 배포됩니다 — 로드맵 #002 참고)
const loadOrder = [
    'modernism.js',
    'alienese.js',
    'doctre.js',
...
| 테스트 대상 | 파일 |
| --- | --- |
| `nne()`, `noe()`, 프리미티브 별칭, 타입 검사 | `test/estreui/alienese-helpers.test.js` |
| `Doctre.matchReplace`, `crashBroker`, `copyPrimitives` | `test/estreui/doctre-match-replace.test.js` |
| PID 파싱 (`getPidComponent`, 스트리퍼, 세터) | `test/estreui/pid-parsing.test.js` |
...
EstreUI에 새 스크립트 파일(예: 플러그인 또는 확장)을 추가하면, `test/estreui/setup.js`의 `loadOrder` 배열에 HTML `<script>` 태그에 나타나는 것과 동일한 위치에 추가하세요. 그런 다음 새 전역 식별자를 `return` 블록에 추가합니다.

## 8. 폴더 구조

```
test/
  estreui/          # 프레임워크(EstreUI.js) 범위 테스트 — 업스트림과 함께 배포
    setup.js
    *.test.js
  project/          # 앱 고유 로직 테스트 — 업스트림 제외 (필요 시 생성)
    setup.js
    *.test.js
```

프레임워크 테스트는 프로젝트 고유 식별자(도메인 이름, API 경로, 브랜드 상수 등)를 포함하지 않아야 합니다 — 업스트림 [SoliEstre/EstreUI.js](https://github.com/SoliEstre/EstreUI.js) 와 함께 배포되기 때문입니다. 앱 고유 테스트는 별도 하위 폴더(`test/project/`)에 두어 경계를 명시적으로 유지하세요.
