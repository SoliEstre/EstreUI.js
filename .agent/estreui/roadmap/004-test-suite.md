...
2. **2단계** — Tier 2 테스트 추가 (jsdom, ~15 케이스). DOM 의존 기능 검증.
3. **3단계** — Tier 3 테스트 추가 (~10 케이스). 더 많은 셋업(목 페이지 구조) 필요하지만 가장 핵심적인 사용자 향 플로우 커버.
4. **4단계** — 리뷰 항목(#001–#007) 각각에 대한 회귀 테스트 추가, 재발 방지.
